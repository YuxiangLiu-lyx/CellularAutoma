...
def main():
    ...
    with open(readme_path, 'w') as f:
        f.write(f"# 3-Channel CNN Convergence Test with L1 Regularization\n\n")
        f.write(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        
        f.write(f"## Experiment Design\n\n")
        f.write(f"- **Objective**: Test if 3-channel CNN can reach 100% accuracy with L1 regularization\n")
        f.write(f"- **Motivation**: Push pruning to the absolute minimum - can we solve Game of Life with just 3 channels?\n")
        f.write(f"- **Configuration**: lambda_l1 = 0.001\n")
        f.write(f"- **Number of runs**: 10\n")
        f.write(f"- **Seeds**: {seeds[0]} to {seeds[-1]}\n")
        f.write(f"- **Parameters per model**: ~22 (3 channels × 3×3 kernel + biases)\n\n")
        ...
        if convergence_epochs:
            ...
            if converged_count > 0:
                ...
                f.write(f"- Channel 2: {np.mean(all_ch2):.3f} ± {np.std(all_ch2):.3f}\n\n")
        ...
        f.write(f"\n## Comparison\n\n")
        f.write(f"| Architecture | Channels | Parameters | Convergence Rate |\n")
        f.write(f"|--------------|----------|------------|------------------|\n")
        f.write(f"| Standard     | 16       | ~177       | 100% (baseline)  |\n")
        f.write(f"| Pruned       | 4        | ~29        | ? (see 4ch test) |\n")
        f.write(f"| **This test**| **3**    | **~22**    | **{converged_count/10*100:.0f}%** |\n\n")
